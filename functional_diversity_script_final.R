...
read.xlsx(file = "functional diversity/data/functional_trait_matrix.xlsx", 
          sheetIndex = 1) -> trait_matrix_raw
...
# We have a positive continuous response variable, so we will use a gamma GLM.
...
fitted(m_brain_mass) -> fitted_values
resid(m_brain_mass) -> residuals
plot(fitted_values, residuals); abline(h = 0, col = "red", lty = 2)
...
# to log-transform the body mass and remodel the data.
...
# Check the residuals vs. predictor again.
plot(trait_matrix_raw$
         brain_mass_residuals_va
       [!is.na(trait_matrix_raw$brain_mass_residuals_va)],
     residuals(m_brain_mass), ylim = c(-2,2))
abline(h = 0, col = "red", lty = 2)
# The residuals look much better now. Proceed with this model.
# 1) Check the fitted values vs. residuals.
fitted(m_brain_mass) -> fitted_values
resid(m_brain_mass) -> residuals
plot(fitted_values, residuals); abline(h = 0, col = "red", lty = 2)
# There are no patterns.
# 2) Residuals vs. the predictor has already been checked. See above.
...
# 5) Compare the model with the null model to see if it's significantly better
# than a model with only a slope or a null model.
drop1(m_brain_mass, test = "Chi") # The term improves the model significantly.
...
# At last, we are done with the correlated trait removal, and we are left with 
# 74 traits. Rename the resulting data frames.
...
read.xlsx(file = "functional diversity/data/trait_weights.xlsx",
           sheetIndex = 1) -> weights
...
# Transpose for use in the dbFD() function.
...
FD_incidences$FRic
100 - (3.259808e-10 /9.232898e-10)*100 # The decline in gamma-scale Functional 
# Richness (FRic) is 65.0%, which is nearly 3.4 times larger than the decline in 
# taxonomic richness (19.2%).
# For this calculation, we could use a maximum of 12 PCoA axes, which explained
# 48.7% of the total variation in the data.
...
# squares results in an uneven case for the study years. Therefore, use GLMMs to
# make the comparison.
c(2,6,7,8,10,11,12,13,14,18,19,20,
  21,22,23,24,25,27,28,30,31,33) -> indexer_shared
study_comms_S4[c(indexer_shared, 34:55), ] -> study_comms_shared
...
# Finally, calculate FD at the grid square or alpha scale.
dbFD(x = trait_matrix_shared, a = study_comms_shared, w = weights$WEIGHT, 
     corr = "cailliez", calc.CWM = T, CWM.type = "all", print.pco = T,
     calc.FDiv = F, m = 4) -> FD_alpha_shared
# The representation of the trait variation is 31.2%.
# Calculate AUC.
...
R_NX(x1) -> x2
AUC_ln_K(x2) # AUC: 0.55 for the shared dataset.

# Extract FRic for the years.
...
# Add the area and relative coordinates.
read.csv2("functional diversity/data/relative_coords.csv", 
          header = T) -> rel_coords
rep(rel_coords$x, 2) -> FRic_df$x
rep(rel_coords$y, 2) -> FRic_df$y
read.csv("functional diversity/data/areas.csv", header = T) -> areas
as.numeric(rep(areas$area, 2)) -> FRic_df$area
str(FRic_df)
rep(areas$square_code, 2) -> FRic_df$square

# Data exploration #
# 1) Check for any zero inflation problem.
which(FRic_df$FRic == 0) # No zeroes. The response is positive continuous.
# 2) Check for outliers.
dotchart(FRic_df$FRic, 
         labels = FRic_df$square, bg = "red") # No outliers.
dotchart(FRic_df$area, 
         labels = FRic_df$square, bg = "green") # No outliers.
# 3) Check the relation between the response and predictors.
# Start with year.
ggplot() +
...
ggplot(data = FRic_df) +
  ...
  geom_smooth(aes(x = area, y = FRic)) +
  theme(panel.border = element_rect(colour = "gray35", fill = NA, size = 0.5)) +
  xlab("Area")
# There seems to be an area effect.
# 4) Check for spatial patterns in FRic values and area values. 
ggplot() +
  geom_point(data = FRic_df[1:22, ], aes(x = x, y = y, col = FRic), 
             size = 5) +
  scale_color_continuous(type = "gradient") +
  guides(col = guide_legend("FRic")) +
  theme(panel.border = element_rect(colour = "gray35", fill = NA, size = 0.5)) +
  xlab("Easting") + ylab("Northing") -> p_check1 # For 1998's data.
ggplot() +
  geom_point(data = FRic_df[23:44, ], aes(x = x, y = y, col = FRic), 
             size = 5) +
  scale_color_continuous(type = "gradient") +
  guides(col = guide_legend("Fric")) +
  theme(panel.border = element_rect(colour = "gray35", fill = NA, size = 0.5)) +
...
# No apparent spatial patterns in the data.

# Do the same for area.
ggplot() +
  geom_point(data = FRic_df[1:22, ], aes(x = x, y = y, col = area), 
             size = 5) +
  scale_color_continuous(type = "gradient") +
  guides(col = guide_legend("Area")) +
  theme(panel.border = element_rect(colour = "gray35", fill = NA, size = 0.5)) +
  xlab("Easting") + ylab("Northing") -> p_check
p_check # No apparent dependence.

# Summary of the data exploration.
# 1) No zero inflation, no outliers.
# 2) There is a strong year effect and potentially non-linear(ish) area effect.
# 3) No spatial dependence.

# Make the formal comparison by using GLMMs.
as.factor(rep((paste("square", 1:22, sep = "_")), 2)) -> FRic_df$ID
scale(FRic_df$area) -> FRic_df$area
m_fric_alpha <- glmmTMB(data= FRic_df, formula= FRic ~ year  + (1 | ID), 
...
# Model validation #
# 1) Plot the fitted values against the residuals.
fitted(m_fric_alpha) -> fitted_values
resid(m_fric_alpha) -> residuals
plot(fitted_values, residuals); abline(h = 0, v = 0, col = "red", lty = 2)
# There may be a heterogeneity problem.
# 2) Plot residuals vs. the only predictor.
boxplot(residuals ~ FRic_df$year)
abline(h = 0, col = "red", lty = 2)
# The model performs nearly equally at both levels. No problem here.
# 3) Check the model for autocorrelation.
par(mfrow = c(1, 2))
...
# 5) Use the DHARMa's utilities to check the model. 
set.seed(13); simulateResiduals(m_fric_alpha, n = 1000) -> sim_fric_alpha
set.seed(13); testDispersion(m_fric_alpha); testZeroInflation(m_fric_alpha)
set.seed(13); testOutliers(m_fric_alpha)
# No problem here.
# Check for the normality of the random effects.
check_model(m_fric_alpha, check = "reqq")
# Looks good.
# 7) Check whether the model is better than the null model.
dredge(m_fric_alpha) # The covariate makes the model better.
# The model is valid.
...
### 3.2.1: PAIRWISE BETA FUNCTIONAL DIVERSITY ####


# We will use the study_comms_shared data frame for the calculations, but we 
# need to split the data frame first.
study_comms_shared[1:22, ] -> comms_1998_shared
study_comms_shared[23:44, ] -> comms_2018_shared
...
# 2018's communities have significantly higher FBsim (p< 0.001***).
wilcox.test(x = as.vector(FD_beta_1998$funct.beta.sne),  
            y = as.vector(FD_beta_2018$funct.beta.sne),
            alternative = "two.sided")
# The difference is significant (p = 0.02803).
wilcox.test(x = as.vector(FD_beta_1998$funct.beta.sor),  
            y = as.vector(FD_beta_2018$funct.beta.sor),
            alternative = "two.sided")
# 2018's communities have significantly higher FBsor (p < 0.001***).


### 3.2.2: DISTANCE DECAY MODELS ####


# Get the distances first.
dist(x = FRic_df[1:21, 3:4], method = "euclidean") -> utm_distances
...
func_decay_select <- function(input){
  ...
  for(i in 1:length(input)){
    ...
    }                     # functional.beta.pair() function is run.
...
# None of the distance decay models (ddms) except for FBsim (slope = 0.011*) and
# FBsor (slope = 0.019*) for 2018 has significant slopes. Let's check if the 
...
# 2) Plot the fitted values versus the residuals.
plot(ddms_2018[[1]]$model$fitted.values, ddms_2018[[1]]$model$residuals)
abline(h = 0, col = "red", lty = 2)
# There seems to be no significant patterns here.
# 3) Plot the residuals against the only predictor in the model to see if there
# are any patterns in the plot. 
plot(utm_distances, 
      ddms_2018[[1]]$model$residuals); abline(h = 0, col = "red", lty = 2)
# No patterns. The model is a valid one.
...
#### 3.2.2.1: COMPARISON OF THE SLOPES ####


# Use the bootstrapped model parameters to compare the slopes of the ddms.
...
set.seed(13); boot.coefs.decay(ddms_1998[[2]], R = 10000) -> boot_ddm_1998_sne
set.seed(13); boot.coefs.decay(ddms_2018[[2]], R = 10000) -> boot_ddm_2018_sne
...
# 2018's communities seem to have a steeper FBsim decay slope.
range(c(boot_ddm_1998_sne$boot.coefs[,2], 
        boot_ddm_2018_sne$boot.coefs[,2])) -> current_range
boxplot(boot_ddm_2018_sne$boot.coefs[,2], col = dark_blue_tr,
        ylim = current_range,
        main = "FBsne Slope Bootstrapped - Blue:1998, Dark blue:2018")
boxplot(boot_ddm_1998_sne$boot.coefs[,2], col = blue_tr,
        ylim = current_range, add = T)
# No apparent difference here. No need to test this.
...
boxplot(boot_ddm_1998_sor$boot.coefs[,2], col = blue_tr,
        ylim = current_range, add = T)
# 2018's communities seem to have a steeper FBsor decay slope.
...
ggplot() +
  ...
  theme(panel.border = element_rect(colour = "gray35", fill = NA, size = 0.5)) +
  theme(axis.title.x = element_text(size = 12)) + 
  ylab("Bootstrapped FBsim DDM slope") + xlab("Year") +
...
ggplot() +
  ...
  theme(panel.border = element_rect(colour = "gray35", fill = NA, size = 0.5)) +
  theme(axis.title.x = element_text(size = 12)) + 
  ylab("Bootstrapped FBsor DDM slope") + xlab("Year") +
...
# p-value for this comparison is 0.06 and the difference is NOT significant.
...
# p-value for this comparison is 0.027*.


#### 3.2.2.2: COMPARISON OF THE INTERCEPTS ####
...
FBsim_intercept$intercept[1:10000] - FBsim_intercept$intercept[10001:20000] -> 
  intercept_dif_FBsim
...
# The difference is NOT significant (p = 0.1178).
data.frame(intercept = c(boot_ddm_1998_sne$boot.coefs[, 1], 
                         boot_ddm_2018_sne$boot.coefs[, 1]), 
           year = as.factor(c(rep("1998", 10000), 
                              rep("2018", 10000)))) -> FBsne_intercept
FBsne_intercept$intercept[1:10000] - FBsne_intercept$intercept[10001:20000] -> 
  intercept_dif_FBsne
length(which(intercept_dif_FBsne < 0)) 
length(which(intercept_dif_FBsne > 0)) # Smaller one.
(length(which(intercept_dif_FBsne > 0)) / 10000)*2
# The difference is NOT significant (p = 0.2798).
...
# The difference is significant (p = 0.023*).
...
ggplot() +
  ...
  annotate(geom = "text", x = 3.45, y = 0.61, label = "AUC = 0.55", 
           col = "firebrick2") +
  annotate(geom = "text", x = 4.8, y = 0.68, label = "AUC = 0.61") +
...
ggplot() +
  ...
  scale_x_continuous(expand = c(0, 0), limits = c(0, 20), 
                     breaks = seq(from = 0, to = 20, by = 1)) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1)) +
  annotate(geom = "text", x = 12, y = 0.81, label = "AUC = 0.76", 
           col = "green4") +
...
mantel(gower_dists_full, gower_dists_4, method = "spearman", 
       permutations = 10000)
# The distance matrices are strongly and positively correlated (rho = 0.84, 
# p< 0.001***)
